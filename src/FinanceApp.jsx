...
const FinanceApp = ({ usuario, onLogout }) => {
  ...
  useEffect(() => {
    const carregarTransacoes = async () => {
      try {
        const res = await fetch(`${API_URL}/api/transacoes/${usuario.id}`, {
        ...
        if (res.ok) {
          ...
          console.error('Erro ao carregar transacoes:', err.error || err.erro || res.status);
        }
      } catch (err) {
        console.error("Erro ao carregar transações:", err);
      }
    };
    if (usuario?.id) carregarTransacoes();
  }, [usuario?.id]);
  ...
  useEffect(() => {
    const carregarCaixinhas = async () => {
      try {
        const res = await fetch(`${API_URL}/api/caixinhas/${usuario.id}`, {
          headers: getAuthHeaders()
        });
        if (res.ok) {
          const dados = await res.json();
          ...
          const err = await res.json().catch(() => ({}));
          console.error('Erro ao carregar caixinhas:', err.error || err.erro || res.status);
        }
      } catch (err) {
        console.error("Erro ao carregar caixinhas:", err);
      }
    };
    if (usuario?.id) carregarCaixinhas();
  }, [usuario?.id]);
  ...
  useEffect(() => {
    const carregarCategorias = async () => {
      ...
      try {
        const res = await fetch(`${API_URL}/api/categorias/${usuario.id}`, {
          headers: getAuthHeaders()
        });
        if (res.ok) {
          const dados = await res.json();
          ...
          const err = await res.json().catch(() => ({}));
          console.error('Erro ao carregar categorias:', err.error || err.erro || res.status);
        }
      } catch (err) {
    ...
    if (usuario?.id) carregarCategorias();
  }, [usuario?.id]);
  ...
  useEffect(() => {
    const carregarContas = async () => {
      ...
      try {
        const res = await fetch(`${API_URL}/api/contas/${usuario.id}`, {
          headers: getAuthHeaders()
        });
        if (res.ok) {
          const dados = await res.json();
          ...
          const err = await res.json().catch(() => ({}));
          console.error('Erro ao carregar contas:', err.error || err.erro || res.status);
        }
      } catch (err) {
  ...
  const salvarRapido = async () => {
    ...
    try {
      if (formRapido.recorrencia === 'nenhuma') {
        ...
        if (!res.ok) {
          const err = await res.json().catch(() => ({}));
          throw new Error(err.error || err.erro || 'Erro ao criar transação');
        }
      } else {
        ...
        for (const trans of transacoesRecorrentes) {
          ...
          if (!res.ok) {
            const err = await res.json().catch(() => ({}));
            throw new Error(err.error || err.erro || 'Erro ao criar recorrência');
          }
        }
  ...
  const deletarTransacao = async (id) => {
    ...
    if (confirmar) {
      try {
        const res = await fetch(`${API_URL}/api/transacoes/${id}`, {
          method: 'DELETE',
          headers: getAuthHeaders()
        });
        if (!res.ok) {
          const err = await res.json().catch(() => ({}));
          throw new Error(err.error || err.erro || 'Erro ao deletar');
        }
        setTransacoes(prev => prev.filter(t => t.id !== id));
        toast.success('Transação deletada!');
      } catch (err) {
        console.error("Erro ao deletar:", err);
        toast.error(err.message || "Erro ao deletar transação");
      }
    }
  };
  ...
  const togglePago = async (id) => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err.error || err.erro || 'Erro ao atualizar');
      }
  ...
  const calcularTotais = (mes) => {
    ...
    const gastos = transacoesMes.filter(t => t.tipo === 'gasto').reduce((sum, t) => sum + (t.valor || 0), 0);
    const receitas = transacoesMes.filter(t => t.tipo === 'receita').reduce((sum, t) => sum + (t.valor || 0), 0);
    const fixos = transacoesMes.filter(t => t.tipo === 'gasto' && t.fixo).reduce((sum, t) => sum + (t.valor || 0), 0);
    const variaveis = transacoesMes.filter(t => t.tipo === 'gasto' && !t.fixo).reduce((sum, t) => sum + (t.valor || 0), 0);
    const naoPagos = transacoesMes.filter(t => t.tipo === 'gasto' && !t.pago).reduce((sum, t) => sum + (t.valor || 0), 0);
  ...
  const calcularVencimentos = () => {
    ...
    const naoPagas = transacoes.filter(t =>
      t.tipo === 'gasto' &&
      !t.pago &&
      (t.data || '').startsWith(mesAtual)
    );

    const vencidas = naoPagas.filter(t => {
      const dataTransacao = new Date((t.data || '') + 'T00:00:00');
      return dataTransacao < hoje;
    });

    const vencem3Dias = naoPagas.filter(t => {
      const dataTransacao = new Date((t.data || '') + 'T00:00:00');
  ...
  const dadosPorCategoria = useMemo(() => {
    ...
    const total = transacoesMes.reduce((sum, t) => sum + (t.valor || 0), 0);
    ...
    transacoesMes.forEach(t => {
      ...
      porCategoria[t.categoria] += t.valor || 0;
    });
  ...
  const getEvolucaoMeses = () => {
    ...
    transacoes.forEach(t => {
      ...
      if (mesTransacao) mesesComDados.add(mesTransacao);
    });
  ...
  const calcularPrevisao = () => {
    ...
    if (gastosFixos === 0) {
      ...
      const somaFixos = fixosMeses.reduce((a, b) => a + b, 0);
      gastosFixos = fixosMeses.length ? (somaFixos / fixosMeses.length) : 0;
    }
    ...
    const mediaVariaveis = variaveisMeses.length ? (variaveisMeses.reduce((a, b) => a + b, 0) / variaveisMeses.length) : 0;
    ...
    if (receitas === 0) {
      ...
      receitas = receitasMeses.length ? (receitasMeses.reduce((a, b) => a + b, 0) / receitasMeses.length) : 0;
    }
  ...
  const editarCaixinha = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/caixinhas/${caixinhaEdit.id}`, {
        method: 'PUT',
        headers: getAuthHeaders(),
        body: JSON.stringify({
          ...formCaixinha,
      ...
      if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        throw new Error(err.error || err.erro || 'Erro ao editar');
      }
      ...
      toast.error(err.message || 'Erro ao editar caixinha');
    }
  };

  // ✅ NOVO: Calcular previsão de conclusão da caixinha
  const calcularPrevisaoCaixinha = (caixinha) => {
    if (!caixinha || !caixinha.parcelas_total) return '';
    if ((caixinha.parcelas_pagas || 0) >= caixinha.parcelas_total) {
      return 'Concluída! 🎉';
    }

    const faltam = caixinha.parcelas_total - (caixinha.parcelas_pagas || 0);
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="bg-red-600/20 border border-red-600/30 rounded-lg p-3">
                ...
                <div className="text-xs text-gray-400 mt-1">
                  R$ {calcularVencimentos().vencidas.reduce((sum, t) => sum + (t.valor || 0), 0).toFixed(2)}
                </div>
              </div>
              ...
              <div className="bg-orange-600/20 border border-orange-600/30 rounded-lg p-3">
                ...
                <div className="text-xs text-gray-400 mt-1">
                  R$ {calcularVencimentos().vencem3Dias.reduce((sum, t) => sum + (t.valor || 0), 0).toFixed(2)}
                </div>
              </div>
              ...
              <div className="bg-green-600/20 border border-green-600/30 rounded-lg p-3">
                ...
                <div className="text-xs text-gray-400 mt-1">
                  R$ {transacoesFiltradas.filter(t => t.pago).reduce((sum, t) => sum + (t.valor || 0), 0).toFixed(2)}
                </div>
              </div>
          ...
          <>
            ...
            {(() => {
              const caixinhasFiltradas = caixinhas.filter(c => {
                const concluida = (c.parcelas_pagas || 0) >= (c.parcelas_total || 0);
              ...
              return (
                <div className="space-y-4">
                  {caixinhasFiltradas.map(c => {
                    ...
                    const progresso = valorTotal ? ((valorPago / valorTotal) * 100).toFixed(1) : 0;
                    const valorParcela = c.parcelas_total ? (valorTotal / (c.parcelas_total || 1)).toFixed(2) : '0.00';
                    const faltam = (c.parcelas_total || 0) - (c.parcelas_pagas || 0);
                      ...
                      <div key={c.id} className="bg-gray-800 rounded-lg p-5">
                            ...
                            <button
                              onClick={async () => {
                                if (window.confirm('Deseja realmente deletar esta caixinha?')) {
                                  try {
                                    const res = await fetch(`${API_URL}/api/caixinhas/${c.id}`, {
                                      method: 'DELETE',
                                      headers: getAuthHeaders()
                                    });
                                    if (!res.ok) {
                                    ...
                                    console.error("Erro ao deletar:", err);
                                    toast.error(err.message || "Erro ao deletar caixinha");
                                  }
                                }
                              }}
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
                          <div>
                            ...
                            <div className="font-bold">R$ {valorTotal.toFixed(2)}</div>
                          </div>
                          <div>
                            <span className="text-gray-400">Pago:</span>
                            <div className="font-bold text-green-400">R$ {valorPago.toFixed(2)}</div>
                          </div>
                          <div>
                          ...
                          <button
                            onClick={async () => {
                              try {
                                ...
                                const res = await fetch(`${API_URL}/api/caixinhas/${c.id}/pagar`, {
                                  method: 'PUT',
                                  headers: getAuthHeaders(),
                                ...
                                if (!res.ok) {
                                  const err = await res.json().catch(() => ({}));
                                  throw new Error(err.error || err.erro || 'Erro ao pagar');
                                }
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    if (!response.ok) {
                      ...
                      throw new Error(err.error || err.erro || 'Erro ao criar');
                    }
